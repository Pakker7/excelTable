(function () {
    ...
    let excelTable = {
        ...
        init : function (obj) {
            ...
            if (Array.isArray(obj.data)) { // 배열 형태로 input 될 경우 에러 표시가 없는 간단한 형태의 엑셀이 출력된다.
                ...
                for (var i = 1; i < this.object.data.excelData.length ; i++) {
                    ...
                    this.simpleSetHeader(subSheetThead, subSheetObject.data.headerAry, this.object.data.excelData[i]); // TODO 서브시트 여러개일 때 index넣어주는 걸로 변경
                    this.simpleInitSheet(subSheetThead, subSheetTbody, this.object.data.excelData[i]);
                    if (subSheetObject.data.errors && subSheetObject.data.errors.length > 0) {
                        this.setError(subSheetTarget, subSheetTbody, false);
                        this.subSheetSetErrorEdge(tbody, this.categorizationErros(false));
                    }
                    subSheetHtmlAry.push($('#' + this.subSheetTmpId).html())
                }
        ...
        simpleInitSheet: function (thead, excelBody, excelData) {
            let that = this;
            ...
            excelData.origin.forEach(function (rowData, idx) {
                ...
                let length = that.simpleGetLength(excelData);
        ...
        simpleSetHeader : function (thead, header, excelData) {
            let that = this;        
            
            let length = this.simpleGetLength(excelData)
        ...
        simpleGetLength : function (excelData) {
